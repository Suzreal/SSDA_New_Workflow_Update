...
- In each relationship object, relationship_type describes the role of the related_person relative to the current person.
- It does NOT describe the current person's role.

Examples:
- If the text says "Juan and Siliby are slaves of Don Francisco Sánchez":
  - Juan should have {"related_person": Francisco_ID, "relationship_type": "enslaver"}
  - Siliby should have {"related_person": Francisco_ID, "relationship_type": "enslaver"}
  - Francisco Sánchez should have {"related_person": Juan_ID, "relationship_type": "slave"}
  - Francisco Sánchez should have {"related_person": Siliby_ID, "relationship_type": "slave"}

Never reverse enslaver/slave direction:
- An enslaved person points to their enslaver with relationship_type="enslaver".
- An enslaver points to an enslaved person with relationship_type="slave".
...
# Removes bad Unicode control characters from strings
def sanitize_string_value(value: str):
    cleaned = CONTROL_CHAR_RE.sub("", value)
    return cleaned, cleaned != value

# Recursively checks the whole extracted JSON object and removes control/null characters from strings
...
def extract_data_from_volume(volume_record_path, instructions_path, training_data_path, keywords = None, match_mode = "or", max_shots = 1000, output_path = None, log_path = "failure_log.json"):
    """
    1. Parses the volume record.
    2. Loads few-shot examples from training_data.json.
    3. Loads extraction instructions from instructions.json.
    4. Loops through every entry.
    5. Calls extract_data_from_entry().
    6. Adds the returned data object to entry["data"].
    7. Optionally saves the processed volume.
    """
...
def check_valid_json(data, id, path):
    """
    Checks whether the model output is valid JSON and contains the people property.
    """    
    json_str = fr'{data}'
    try:
...
def log_failed_people(data, id, path):
    """
    Checks that every person has both id and name. People missing those fields are removed and logged.
    """    
    json_str = fr'{data}'
...
def log_failed_events(data, id, path):
    """
    Invalid events or invalid principals are removed/logged.
    """    
    json_str = fr'{data}'
    data = json.loads(json_str)
...
def log_failed_relationships(data, id, path):
    """
    This removes malformed relationships before the reciprocity fixer runs.
    """   
    json_str = fr'{data}'
    data = json.loads(json_str)
