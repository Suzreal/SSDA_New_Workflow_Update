...
from normalize import normalize_entry, normalize_volume
from utility import parse_volume_record, generate_training_data, collect_instructions
...
OUTPUT_FILENAME = "Generated_0023_0034_merge_V4.json" # Output file
...
MATCH_MODE = "or"
NORMALIZATION_MAX_SHOTS = 1
EXTRACTION_MAX_SHOTS = 2
...
# LLM repair settings
USE_LLM_REPAIR = True # always turn this on
LLM_REPAIR_MODEL = "gpt-4o" # Change the model
...
# Always turn this on, as we now shift to this direction of combining normalization and repairing together
LLM_REPAIR_RETURNS_NORMALIZED = True

# Safety fallback: if the repair model fails to provide normalized text for an entry,
# normalize only that missing entry with normalize.py instead of re-normalizing the whole volume.
NORMALIZE_MISSING_ENTRIES_ONLY = True
...
def prepare_input_file(input_path: Path) -> Path:
    """Optionally convert Archivault input into legacy-compatible format.
    This is optional now, it will not harm anything
    """
...
def build_volume_record_from_pages(
    ...
    Essentially, this function truns things into:
            {
        "type": "baptism",
        "country": "United States",
        "state": "Florida",
    ...
    Each page becomes one page object, and the LLM repair step
...
# The system prompt for LLM repairing + Normalization
...
""".strip()
    ...
    if LLM_REPAIR_RETURNS_NORMALIZED:
        normalized_instruction = (
            'For each repaired entry, return BOTH "raw" and "normalized". '
            'The "raw" field must preserve the repaired original transcription as much as possible. '
            'The "normalized" field must be a readable Spanish normalized transcription in the same style as training_data.json. '
            'Preserve all factual content: person names, place names, titles, occupations, dates, age phrases, '
            'kinship phrases, legitimacy phrases, free/enslaved status, enslaver/slave phrases, godparents, witnesses, '
            'officiant phrases, and closing/signature formulae. '
            'Do not invent missing words, names, dates, people, relationships, or facts. '
            'Fix obvious OCR spacing, line-break hyphenation, punctuation, capitalization, historical spelling, and accents only when confidence is high. '
            'Improve sentence boundaries for readability, but do not summarize, shorten, or modernize the record. '
            'Remove page headers, marginalia, decorative marks, page numbers, and obvious non-record junk only if clearly not part of the record. '
            'Keep the normalized text in Spanish. Do not translate it into English. '
            'If a word or name is unclear, keep the visible transcription rather than guessing. '
            'The normalized field should be non-empty for every entry unless the raw entry is truly unusable.'
        )
    else:
            ...
            "entries": [
                {
                    ...
                    "normalized": "readable normalized Spanish transcription, or empty string only if normalization was not requested",
                }
            ],
            "dropped_fragments": [
                {
...
# =============================================================================
# Very important, please change the reasoning effort if you are using a reasoning model
# =============================================================================
...
def window_pages(pages: List[Dict[str, str]]) -> List[List[Dict[str, str]]]:
    """
    Create overlapping page windows to let the model see boundaries
...
def entry_sort_key(entry: Dict[str, Any]) -> Tuple[str, int, str]:
    # a sorting key based on entry IDs like 0017-01
...
# =============================================================================
# This is the function we want to fix
# =============================================================================
...
# =============================================================================
# Optional normalization fallback wrapper, for most cases, this will be skipped, this is the old normalization script
# =============================================================================
...
def normalize_volume_if_needed(
    ...
    """
    If LLM repair produced normalized text, skip the separate normalization pass.

    Cost-saving behavior:
    - Best case: every repaired entry already has "normalized", so normalize.py is skipped.
    - Fallback case: if a few entries are missing normalized text, normalize only those entries.
    - Final fallback: if NORMALIZE_MISSING_ENTRIES_ONLY is False, run normalize_volume() as before.
    """
    entries = volume_record.get("entries", [])
    missing_indices = [
        i for i, e in enumerate(entries)
        if not isinstance(e.get("normalized"), str) or not e.get("normalized", "").strip()
    ]
    ...
    if not missing_indices:
        print("[NORMALIZE] skipped because LLM repair already returned normalized text for every entry.")
        return volume_record

    if not NORMALIZE_MISSING_ENTRIES_ONLY:
        print(
            f"[NORMALIZE] {len(missing_indices)} entries missing normalized text; "
            "running full normalize_volume() fallback."
        )
    ...
    print(
        f"[NORMALIZE] {len(missing_indices)} entries missing normalized text; "
        "normalizing only missing entries."
    )

    _, volume_metadata = parse_volume_record(volume_record)
    ...
    for i in missing_indices:
        volume_record["entries"][i]["normalized"] = normalize_entry(
            volume_record["entries"][i],
            volume_metadata,
            examples,
            instructions,
        )
